...
async def _get_or_create_cart_id(db: AsyncSession, user_id: str) -> str:
    ...
    r = await db.execute(q, {"uid": user_id})
    ...
    r2 = await db.execute(ins, {"uid": user_id})
...
async def _get_cart_payload(db: AsyncSession, cart_id: str) -> Dict[str, Any]:
    ...
    NOTE: price is volatile until checkout; we do not lock/decrement here.
    """
    q = text("""
        select
          ci.id as item_id,
    ...
    for r in rows:
        ...
        line_total = unit_price * int(r["qty"])
        total += line_total
        items.append({
            "item_id": r["item_id"],
            "meal_id": r["meal_id"],
...
async def add_item(
    ...
    # current qty in cart for this meal
    cur_q_q = text("""
...
async def update_item_qty(
    ...
    # ensure item belongs to this cart & fetch meal
    own_q = text("""
...
async def checkout_cart(
    ...
    # fetch items and lock meals (to prevent race on surplus)
    items_q = text("""
    ...
    # all items must be from the same restaurant? (common UX)
    ...
    # validate surplus and compute total using surplus_price
    total = 0.0
    for r in rows:
    ...
    create_order_q = text("""
        ...
        returning id
    """)
    ores = await db.execute(create_order_q, {"uid": user_id, "rid": restaurant_id})
    order_id = ores.mappings().first()["id"]

    # add items, decrement surplus
    for r in rows:
